...
**File:** [`packages/unisig/src/runes.ts`](packages/unisig/src/runes.ts:7-15)
...
try {
  const count = state(0); // No adapter set
} catch (error) {
  if (error instanceof NoAdapterError) {
    console.error('Please set an adapter first');
  }
}
```
...
**File:** [`packages/unisig/src/runes.ts`](packages/unisig/src/runes.ts:87-94)
...
**File:** [`packages/unisig/src/runes.spec.ts`](packages/unisig/src/runes.spec.ts)
...
- No changes to behavior (only additions)
...
```typescript
// Now you can import and use NoAdapterError
import { NoAdapterError } from 'unisig';
