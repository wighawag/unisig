import { Reactive, type ReactivityAdapter, type Listener, type Unsubscribe } from 'unisig'
...
class PlayerStore {
   ...
   * Subscribe to events
   */
  on<K extends keyof PlayerEvents>(event: K, listener: Listener<PlayerEvents[K]>): Unsubscribe {
  ...
  incrementScore(id: string, amount: number = 1): void {
    ...
    if (!player) return
    player.score += amount
    this.$.triggerItemProp('players', id, 'score', 'player:scored', { id, score: player.score })
  }
  ...
  updateLevel(id: string, level: number): void {
    ...
    if (!player) return
    player.level = level
    this.$.triggerItemProp('players', id, 'level', 'player:levelUp', { id, level })
  }
  ...
  levelUp(id: string): void {
    ...
    if (!player) return
    player.level++
    this.$.triggerItemProp('players', id, 'level', 'player:levelUp', { id, level: player.level })
  }
